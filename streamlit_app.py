...
city_traffic_params = {
    ...
    "Personnalisé": { # Option pour les paramètres personnalisés
        "rho_max_global": 300,
        "congestion_factor_morning_peak": 1.5,
...
# --- Paramètres de simulation et fonctions de trafic ---
# Les variables globales v_max, rho_max_global, time_step, mu_T, sigma_T, et les paramètres de congestion
# sont maintenant définis en fonction de la ville choisie dans l'interface Streamlit.
time_step = 0.5
...
# congestion_factor_morning_peak, congestion_factor_lunch_peak, congestion_factor_evening_peak
# sont maintenant définis en fonction de la ville choisie.
...
def velocity_spatial(x, t, road_length, v_max, rho_max, road_length_param): # Removed unused T parameter
    rho_value = rho_gaussiennes_spatiale(x, t, road_length_param, rho_max, # road_length_param instead of road_length in rho_gaussiennes_spatiale
                                        sigma_seconds_peak1=sigma_seconds_peak1, sigma_seconds_peak2=sigma_seconds_peak2, sigma_seconds_peak3=sigma_seconds_peak3,
                                        peak1_seconds=peak1_seconds, peak2_seconds=peak2_seconds, peak3_seconds=peak3_seconds)
    return v_max * (1 - rho_value / rho_max)

def simulate_vehicle_spatial(road_length, v_max, route_rho_max, road_length_param, time_step, journey_start_time=None): # Renamed rho_max to route_rho_max, added road_length_param
    position = 0
    time_elapsed_in_simulation = 0
    segment_travel_time = 0

    while position < road_length:
        current_time_for_density = journey_start_time + time_elapsed_in_simulation if journey_start_time is not None else time_elapsed_in_simulation # Correctly using journey_start_time
        v = velocity_spatial(position, current_time_for_density, road_length, v_max, route_rho_max, road_length_param) # Using current_time_for_density and road_length_param
        if v <= 0:
            return float('inf')
    ...
    return segment_travel_time # Removed a posteriori congestion factor

def simulate_journey(start_node, end_node, start_time, matrix, v_max, G, time_step): # G au lieu de rho_max_matrix et period_matrix
    ...
    while current_node != end_node:
        next_node = end_node # Direct path, for time matrix calculation this is correct for each pair
        if next_node not in G.neighbors(current_node): # Handle cases where nodes are not directly connected in sparse graphs if you move to that
            return float('inf') # Or handle differently, e.g., find a path if needed

        road_length = G[current_node][next_node]['length']
        route_rho_max = G[current_node][next_node]['rho_max']
        road_length_param = road_length # Use road_length as road_length_param
...
def generate_graph_data(taille_matrice, borne_min, borne_max, rho_max_global, congestion_factor_morning_peak, congestion_factor_lunch_peak, congestion_factor_evening_peak, mu_T, sigma_T):
    ...
    road_network = nx.complete_graph(taille_matrice) # Using complete graph for time matrix
    ...
    for u, v in road_network.edges():
        ...
        road_importance_congestion_factor = max(0.5, 2 - normalized_distance * 3) # Reconsider this factor

        combined_congestion_factor = congestion_factor * road_importance_congestion_factor
        route_rho_max = base_rho_max * combined_congestion_factor # Apply to rho_max

        route_period = int(max(1, np.random.normal(mu_T, sigma_T))) # Keep period as is, or adjust if needed based on your model logic

        # --- Ajout des attributs à l'arête du graphe ---
        G.add_edge(u, v, length=weight, rho_max=route_rho_max, period=route_period) # rho_max adjusted
        G.add_edge(v, u, length=weight, rho_max=route_rho_max, period=route_period)

    np.fill_diagonal(matrix, 0)

    time_matrix = None # No precalculation here
    rho_max_matrix = None # Not returning rho_max_matrix anymore

    seed_value = random.randint(0, 1000)
    return adj_matrix, G, start_point, sources, matrix, None, time_matrix, rho_max_matrix, seed_value # period_matrix is None, rho_max_matrix is None
...
if ville_choisie == "Personnalisé":
    ...
    v_max_user = st.sidebar.slider("Vitesse Maximale (km/h)", 10, 30, int(city_traffic_params["Personnalisé"]["v_max"]), step=1) # Slider pour v_max en entier
    mu_T_user = st.sidebar.slider("Mu (Temps Pic Trafic, secondes)", 0, 86400, city_traffic_params["Personnalisé"]["mu_T"], step=3600) # Slider pour mu_T (heures)
    sigma_T_user = st.sidebar.slider("Sigma (Étalement Pic Trafic, secondes)", 1800, 14400, city_traffic_params["Personnalisé"]["sigma_T"], step=3600) # Slider pour sigma_T (heures)
...
if st.button("Visualiser/Désafficher la Matrice des Temps de Parcours (pour l'heure choisie)"): # Titre mis à jour
    st.session_state.show_time_matrix = not st.session_state.show_time_matrix
if st.session_state.show_time_matrix:
    st.write("Matrice des temps de parcours (pour l'heure choisie) :") # Texte mis à jour
    st.dataframe(time_matrix_user_time) # Afficher time_matrix_user_time


#if 'show_cost_matrix' not in st.session_state:
   # st.session_state.show_cost_matrix = False
#if st.button("Visualiser/Désafficher la Matrice des Coûts"):
 #   st.session_state.show_cost_matrix = not st.session_state.show_cost_matrix
#if st.session_state.show_cost_matrix:
...
def ant_colony_optimization(cost_matrix, time_matrix, distance_matrix, num_ants, num_iterations, alpha_aco, beta_aco, evaporation_rate, q, start_point_aco):
    ...
    for iteration in range(num_iterations):
        ...
        for ant in range(num_ants):
            ...
            all_paths.append(visited) # Corrected typo: all_paths.append(visited)
            all_distances.append(path_distance)
            all_times.append(path_time)

            if path_distance < best_distance: # Using distance as optimization criteria for best path, change to cost or time if needed
                best_path = visited
                best_distance = path_distance
                best_time = path_time

        pheromones *= (1 - evaporation_rate)
        for path, distance in zip(all_paths, all_distances): # Using distance here as well for pheromone update - consider cost if that's your goal
            path_cost = 0
